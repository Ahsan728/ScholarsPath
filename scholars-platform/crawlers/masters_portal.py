"""
MastersPortal.com Crawler (Sitemap → Detail Pages)
====================================================
Strategy:
  1. Fetch sitemap to collect all /studies/ programme URLs
  2. Scrape each detail page with httpx — they are server-rendered with JSON-LD
  3. Extract structured data from JSON-LD blocks and meta tags

No Playwright needed — individual programme pages have server-side HTML.
...
logger = logging.getLogger("masters_portal")
...
FIELD_CATEGORY = {
    ...
    "artificial": "cs_ai", "machine learning": "cs_ai",
    "cybersecurity": "cs_ai", "information": "cs_ai",
...
def fetch_xml(client: httpx.Client, url: str) -> Optional[ET.Element]:
    """Fetch and parse an XML (or gzipped XML) sitemap."""
    try:
...
def collect_programme_urls(client: httpx.Client, max_urls: int = 2000) -> list[str]:
    """Walk the sitemap tree and collect /studies/ programme URLs."""
    ...
    for sitemap_url in SITEMAP_CANDIDATES:
        ...
        if sub_sitemaps:
            for loc_el in sub_sitemaps:
                ...
                if not sub_url:
                    continue
                # Only fetch sitemaps that look programme-related
                if any(kw in sub_url for kw in ["studie", "programme", "course", "master"]):
            ...
            # Flat sitemap — look for /studies/ URLs directly
...
def parse_detail_page(html: str, url: str) -> Optional[RawProgram]:
    """Extract programme data from a detail page using JSON-LD and meta tags."""
    from bs4 import BeautifulSoup
    soup = BeautifulSoup(html, "html.parser")

    # ── JSON-LD (most reliable — server-rendered for SEO) ──────────────────
    for script in soup.find_all("script", type="application/ld+json"):
        try:
            ...
            for entry in entries:
                t = entry.get("@type", "")
                if t not in ("EducationalOccupationalProgram", "Course",
                             "CollegeOrUniversity", "Event"):
                    continue
                ...
                if isinstance(provider, dict):
                    ...
                    university = provider
                    country = extract_country(html[:5000])
                    city = country
                ...
                duration = parse_duration(str(duration_raw))

                # Description
                desc = entry.get("description") or f"Master's program at {university}, {country}."
    ...
    name = re.sub(r"\s*[-|]\s*MastersPortal.*$", "", raw_name, flags=re.I).strip()
    if not name or len(name) < 5:
        return None
    ...
    uni_el = (
        soup.find(class_=re.compile(r"university|institution|school|provider", re.I)) or
        soup.find("meta", {"name": "author"})
    )
    university = (
        uni_el.get("content", uni_el.get_text(strip=True)) if uni_el else "European University"
    )
    ...
    return RawProgram(
        ...
        deadline=None,
        scholarship_available=False,
        description=desc[:500] if desc else f"Master's program at {university}, {country}.",
        requirements=[
            "Bachelor's degree in a relevant field",
            "English language proficiency (IELTS 6.0+)",
        ],
...
class MastersPortalCrawler(BaseProgramCrawler):
    SOURCE_NAME = "masters_portal"
    CRAWL_DELAY = 0.4
    MAX_PROGRAMMES = 2000
    ...
    def fetch(self) -> list[RawProgram]:
        ...
        with httpx.Client(headers=HEADERS, follow_redirects=True) as client:
            ...
            if not urls:
                self.logger.warning(
                    ...
                    "MastersPortal may have changed their sitemap structure."
                )
                return []
        ...
        self.logger.info(f"MastersPortal total: {len(programs)} programs")
        return programs
...
if __name__ == "__main__":
    ...
    crawler = MastersPortalCrawler()
    items = crawler.fetch()
    print(f"Fetched {len(items)} programs from MastersPortal.com")
    for p in items[:5]:
